...
export function saveWhiteboardExercise(form) {
  ...
  const record = {
    ...
    statementFr: normalizeRichHtml(form.statementFr),
    officialResult: normalizeRichHtml(form.officialResult),
    dataItems: sanitizeRichList(form.dataItems).slice(0, 10),
    dataItemsFr: sanitizeRichList(form.dataItemsFr).slice(0, 10),
    conditionItems: sanitizeRichList(form.conditionItems).slice(0, 10),
    conditionItemsFr: sanitizeRichList(form.conditionItemsFr).slice(0, 10),
    clarificationItems: sanitizeRichList(form.clarificationItems).slice(0, 10),
    clarificationItemsFr: sanitizeRichList(form.clarificationItemsFr).slice(0, 10),
    taskItems: sanitizeRichList(form.taskItems).slice(0, 10),
    taskItemsFr: sanitizeRichList(form.taskItemsFr).slice(0, 10),
...
function normalizeWhiteboardExerciseRecord(record) {
  ...
  return {
    ...
    statementFr: normalizeRichHtml(nextRecord.statementFr || nextRecord.statement_fr),
    officialResult: normalizeRichHtml(nextRecord.officialResult),
    dataItems: sanitizeRichList(nextRecord.dataItems),
    dataItemsFr: sanitizeRichList(nextRecord.dataItemsFr || nextRecord.data_items_fr),
    conditionItems: sanitizeRichList(nextRecord.conditionItems),
    conditionItemsFr: sanitizeRichList(nextRecord.conditionItemsFr || nextRecord.condition_items_fr),
    clarificationItems: sanitizeRichList(nextRecord.clarificationItems),
    clarificationItemsFr: sanitizeRichList(nextRecord.clarificationItemsFr || nextRecord.clarification_items_fr),
    taskItems: sanitizeRichList(nextRecord.taskItems),
    taskItemsFr: sanitizeRichList(nextRecord.taskItemsFr || nextRecord.task_items_fr),
