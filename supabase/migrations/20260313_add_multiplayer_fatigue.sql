alter table public.mp_match_players
  add column if not exists fatigue_count integer not null default 0;

alter table public.mp_match_players
  drop constraint if exists mp_match_players_fatigue_count_check;

alter table public.mp_match_players
  add constraint mp_match_players_fatigue_count_check check (fatigue_count >= 0);
