-- Multiplayer snapshot exercise fields + opening hand fix
-- Date: 2026-03-12

alter table public.mp_match_constructs
  add column if not exists exercise_statement text,
  add column if not exists exercise_final_answer text;

update public.mp_match_constructs mc
set
  exercise_statement = e.statement,
  exercise_final_answer = e.final_answer
from public.competitive_exercises e
where e.id = mc.source_exercise_id
  and (mc.exercise_statement is null or mc.exercise_final_answer is null);
