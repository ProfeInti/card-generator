-- Add ingenuity cost to constructs and multiplayer snapshots
-- Date: 2026-03-12

alter table public.competitive_constructs
  add column if not exists ingenuity_cost integer not null default 0;

alter table public.competitive_constructs
  drop constraint if exists competitive_constructs_ingenuity_cost_check;

alter table public.competitive_constructs
  add constraint competitive_constructs_ingenuity_cost_check
  check (ingenuity_cost >= 0);

alter table public.mp_match_constructs
  add column if not exists ingenuity_cost integer not null default 0;

alter table public.mp_match_constructs
  drop constraint if exists mp_match_constructs_ingenuity_cost_check;

alter table public.mp_match_constructs
  add constraint mp_match_constructs_ingenuity_cost_check
  check (ingenuity_cost >= 0);
