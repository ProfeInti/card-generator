-- Multiplayer technique card snapshot + participant refresh support
-- Date: 2026-03-12
...
update public.mp_match_cards mc
set
  technique_name = t.name,
  technique_topic = t.topic,
  technique_subtopic = t.subtopic,
  technique_effect_type = t.effect_type,
  technique_effect_description = t.effect_description,
  technique_worked_example = t.worked_example
from public.competitive_techniques t
where mc.source_type = 'technique'
  and mc.source_technique_id = t.id
  and (
    mc.technique_name is null
    or mc.technique_effect_description is null
    or mc.technique_effect_type is null
  );
...
declare
  ...
  v_opponent_position_base integer;
begin
  select *
  into v_room
  ...
  loop
    ...
    loop
      ...
      select coalesce(max(c.position_index), 0)
      into v_opponent_position_base
      from public.mp_match_cards c
      where c.match_id = v_match_id
        and c.owner_user_id = v_opponent;
      ...
      select
        ...
        t.worked_example,
        'deck',
        v_opponent_position_base + row_number() over (order by s.solution_path, s.step_order),
        v_match_construct_id,
        true
      from public.competitive_construct_steps s
      join public.competitive_techniques t on t.id = s.technique_id
      where s.construct_id = v_construct_id
      order by s.solution_path, s.step_order;
    end loop;
